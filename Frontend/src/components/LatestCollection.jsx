...
const LatestCollection = () => {
  ...
  return (
    <section className="container mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold text-center mb-8">Latest Collection</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {latestProducts.map((product) => {
          ...
          // Check wishlist status from context state
          ...
          return (
            <div key={product._id} className="flex flex-col">
              <div className="relative bg-white rounded-lg shadow p-4 flex flex-col items-stretch hover:shadow-lg transition group h-full">
                <button
                  // Use the context function to toggle wishlist item
                ...
                <Link to={`/product/${product._id}`} className="flex-1 flex flex-col">
                  <div className="w-full h-60 bg-gray-100 rounded-lg overflow-hidden mb-4 flex items-center justify-center">
                    <img src={product.image?.[0]} alt={product.name} className="object-contain w-full h-full" />
                  </div>
                  <h3 className="text-lg font-semibold text-gray-800 mb-1 truncate">{product.name}</h3>
                  <p className="text-indigo-600 font-bold text-xl mb-2">{currency}{product.price}</p>
                </Link>
